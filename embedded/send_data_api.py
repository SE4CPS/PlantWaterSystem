...
def fetch_next_row(last_id):
    ...
    Returns a tuple corresponding to a row or None if no row is found.
    """
    try:
        ...
        conn.close()
        if row:
            logging.debug(f"Fetched row with id {row[0]}")
        else:
            logging.debug("No new row found to send.")
        return row
    except Exception as e:
...
def send_all_available(url):
    """
    Loop through the database and send each unsent row one by one.
    """
    global LAST_SENT_ID
    while True:
        ...
        if not (success or duplicate):
            logging.error(f"Failed to send reading {reading['id']}. Stopping further attempts.")
            return False
...
def auto_send():
    """
    Auto-send endpoint (internal service). It optionally accepts an "after" timestamp
    to reset the starting point. It then sends every unsent row to the external backend
    defined in BACKEND_API_SEND_DATA.
    """
    global LAST_SENT_ID
...
def manual_send():
    """
    Manual send endpoint (internal service). It requires an "after" timestamp in the request,
    resets LAST_SENT_ID accordingly, and then sends unsent rows using BACKEND_API_SEND_CURRENT.
    """
    global LAST_SENT_ID
    req = request.get_json() or {}
    after_str = req.get("after")
