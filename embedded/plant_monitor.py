...
try:
    ...
    logging.error(f"Failed to initialize I2C and ADS1115: {e}")
    sys.exit(1)

# Start `send_data_api.py` subprocess with error handling
try:
    api_process = subprocess.Popen(["python3", "send_data_api.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
except Exception as e:
...
def convert_adc_to_moisture(adc_value):
    ...
    min_adc = 14000
    max_adc = 26000
    if adc_value < min_adc or adc_value > 32767:
        return 0  # Treat invalid readings as dry
...
def read_sensor_channel(sensor):
    ...
    try:
        ...
        logging.error(f"Error reading sensor {sensor['analog']}: {e}")
        return 0, 0, "Error"
...
def read_sensors():
    ...
    for index, sensor in enumerate(SENSORS, start=1):
        ...
        log_entry = f"{index},{moisture_level:.2f},{digital_status}\n"
        with open("sensor_log.log", "a") as log_file:
            log_file.write(log_entry)
        logging.info(f"Sensor {index} - ADC: {adc_value}, Moisture: {moisture_level:.2f}%, Status: {digital_status}")
...
def manage_data_retention():
    ...
    try:
        ...
        cursor.execute("DELETE FROM moisture_data WHERE timestamp < ?", (cutoff_date,))
        conn.commit()
        logging.info(f"Old records older than {RETENTION_DAYS} days removed.")
    except sqlite3.Error as e:
        logging.error(f"Error managing data retention: {e}")
