...
@add_moisture_data.post("/api/sensor/data", response_model=MoistureDataSchema)
def add_moisture_entry(sensor: MoistureDataSchema, service: SensorService = Depends(get_service)):
    try:
        # Call the service layer to add sensor moisture data
        response = service.receive_moisture_data(sensor)
